...
{
    SdaiEntity entity = 0;
    engiGetAttrTraits(attr, (char**)0, &entity, 0, 0, 0, 0, 0, 0);
...
double Context::GetUnitScale(SdaiInstance unit, const char* unitKind)
{
    double scale = 1;
    if (unit) {
        scale = getUnitInstanceConversionFactor(unit, (char**) nullptr, (char**) nullptr, (char**) nullptr, (char**) nullptr);
    }
    else if (unitKind) {
...
Facet::Matched FacetAttribute::MatchImpl(SdaiInstance inst, Context& ctx)
{
    ...
    if (auto attrName = m_name.GetSimpleValue()) {
        ...
        for (SdaiInteger i = 0; i < N; i++) {
            ...
            const char* name = 0;
            engiGetAttrTraits(attr, &name, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
{
    const char* name = 0;
    engiGetAttrTraits(prop, &name, NULL, NULL, NULL, nullptr, nullptr, NULL, NULL);
    if (!m_baseName.Match(name, false, ctx)) {
        return true; //not to test
    }
