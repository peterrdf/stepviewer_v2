...
/*virtual*/ void CIFCOpenGLView::_postDraw() /*override*/
{
...
void CIFCOpenGLView::DrawBoundingBoxes(_model* pModel)
{
	if (pModel == nullptr) {
		return;
	}
	...
	for (auto pGeometry : pModel->getGeometries()) {
		...
		auto pInstance = pGeometry->getInstances().front();
		...
		if (pGeometry->isPlaceholder()) {
			assert(!ifcGeometry->getMappedGeometries().empty());
