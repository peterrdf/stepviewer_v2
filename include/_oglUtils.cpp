...
/*virtual*/ void _oglView::_load(bool bUpdateCameraSettings/* = true*/)
{
	...
	_load(getController()->getModels(), m_worldBuffers);
...
/*virtual*/ void _oglView::_draw(CDC* pDC)
{
	...
	_drawFaces();
	_drawFacesPolygons();
	_drawConceptualFacesPolygons(m_worldBuffers);
	_drawLines(m_worldBuffers);
	_drawPoints();
...
/*virtual*/ void _oglView::_drawFaces(_oglBuffers& oglBuffers, bool bTransparent, bool bApplyApplicationSettings/* = true*/)
{
	...
	for (auto itCohort : oglBuffers.cohorts()) {
		...
		for (auto pGeometry : itCohort.second) {
			...
			auto pModel = getController()->getModelByOwlInstance(pGeometry->getOwlModel());
...
void _oglView::_onMouseMoveEvent(UINT nFlags, CPoint point)
{
	...
	if (m_pPointedInstance != nullptr) {
		...
		if (timeSpan >= 200) {
			auto pModel = getController()->getModelByOwlInstance(m_pPointedInstance->getOwlModel());
