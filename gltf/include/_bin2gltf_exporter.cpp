...
namespace _bin2gltf
{
	_exporter::_exporter(OwlModel iModel, const char* szOutputFile, bool bEmbeddedBuffers)
		: _log_client()
		, m_iModel(iModel)
	...
	_exporter::_exporter(const char* szInputFile, const char* szOutputFile, bool bEmbeddedBuffers)
		: _exporter((OwlModel)0, szOutputFile, bEmbeddedBuffers)
	{
	...
	/*virtual*/ bool _exporter::preExecute()
	{
		...
		for (const auto& itInstanceDefaultState : m_mapInstanceDefaultState) {
			if (!itInstanceDefaultState.second) {
				continue;
			}

			auto itInstanceTransformations = m_mapInstanceTransformations.find(itInstanceDefaultState.first);
			const vector<_matrix*>* pvecTransformations = itInstanceTransformations != m_mapInstanceTransformations.end() ? &(itInstanceTransformations->second) : nullptr;

			//#todo
			/*auto pNode = new _node(itInstanceDefaultState.first, pvecTransformations);
		...
		return !m_mapInstanceDefaultState.empty();
	}
	...
	/*virtual*/ void _exporter::GetInstancesDefaultState()
	{
		...
		for (auto& itInstanceDefaultState : m_mapInstanceDefaultState) {
			...
			if (!GetInstanceGeometryClass(itInstanceDefaultState.first) ||
				...
				VERIFY_INSTANCE(iInstanceClass);
	...
	void _exporter::GetInstanceDefaultStateRecursive(OwlInstance iInstance)
	{
